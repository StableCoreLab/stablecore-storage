...
namespace StableCore::Storage
{
    namespace
    {
        ...
        {
        public:
            explicit Lexer(std::wstring source) : source_(std::move(source))
            {
            }
            ...
            Token Next()
            {
                ...
                {
                    case L'+':
                        return Token{Token::Kind::Plus, L"+"};
                    case L'-':
                        return Token{Token::Kind::Minus, L"-"};
                    case L'*':
                        return Token{Token::Kind::Star, L"*"};
                    case L'/':
                        return Token{Token::Kind::Slash, L"/"};
                    case L',':
                        return Token{Token::Kind::Comma, L","};
                    case L'(':
                        return Token{Token::Kind::LParen, L"("};
                    case L')':
                        return Token{Token::Kind::RParen, L")"};
                    default:
                        return {};
                }
            }
    ...
    ErrorCode EvaluateComputedColumn(const SCComputedColumnDef& column,
                                     ISCComputedContext* context,
    ...
    bool DoesDependencySetIntersect(const SCComputedDependencySet& dependencies,
                                    const SCChangeSet& SCChangeSet) noexcept
    {
