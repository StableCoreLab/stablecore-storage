    ...
    SCVersionGraph* outGraph)
{
    ...
    AppendVersionNode(graph.nodes, minimumVersion, L"v" + std::to_wstring(minimumVersion), L"Minimum supported schema version.");
    AppendVersionNode(graph.nodes, currentVersion, L"v" + std::to_wstring(currentVersion), L"Current schema version.");
...
ErrorCode EvaluateCompatibilityWindow(
    const SCVersionGraph& graph,
...
ErrorCode EvaluateOpenDecision(
    const SCVersionGraph& graph,
    std::int32_t schemaVersion,
    bool cleanShutdown,
    SCOpenDecision* outDecision)
{
    ...
    if (schemaVersion > graph.compatibilityWindow.maxReadableVersion)
    {
        ...
        decision.reason = graph.compatibilityWindow.readOnlyAllowed
            ? L"Schema version is newer than the supported write window; open read-only."
            : L"Schema version is newer than the supported read window.";
        *outDecision = std::move(decision);
        return SC_OK;
    }
    ...
    {
        decision.mode = graph.compatibilityWindow.upgradeAllowed ? SCOpenMode::UpgradeRequired : SCOpenMode::ReadOnly;
        decision.needsUpgrade = graph.compatibilityWindow.upgradeAllowed;
        decision.readOnlyOnly = true;
        decision.writable = false;
        decision.reason = graph.compatibilityWindow.upgradeAllowed
            ? L"Schema version is within the readable window but requires upgrade before write."
            : L"Schema version is readable but not writable in the current compatibility window.";
        *outDecision = std::move(decision);
        return SC_OK;
    }
