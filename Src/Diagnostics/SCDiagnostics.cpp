...
namespace StableCore::Storage
{
    ...
    ErrorCode BuildStorageHealthReport(ISCDatabase* database,
                                       ...
                                       SCStorageHealthReport* outReport)
    {
        ...
        } else
        {
            ...
            report.diagnostics.push_back(SCDiagnosticEntry{
                SCDiagnosticSeverity::Info,
                L"recovery",
                L"Undo/redo journal should be replay-safe before product "
                L"startup proceeds.",
            });
        }
                                ...
                                std::wstring* outText)
    {
        ...
        for (const auto& change : SCChangeSet.changes)
        {
            ...
            switch (change.kind)
            {
                case ChangeKind::FieldUpdated:
                    stream << L" FieldUpdated";
                    break;
                case ChangeKind::RecordCreated:
                    stream << L" RecordCreated";
                    break;
                case ChangeKind::RecordDeleted:
                    stream << L" RecordDeleted";
                    break;
