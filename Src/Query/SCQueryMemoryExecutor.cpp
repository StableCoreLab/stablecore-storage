...
namespace StableCore::Storage
{
    namespace
    {
        ...
        {
            if (database == nullptr || outMatchedRecords == nullptr ||
                outResult == nullptr)
            {
                return SC_E_POINTER;
            }
        ...
        ErrorCode MaterializeCursor(std::vector<SCRecordPtr> matchedRecords,
                                    const QueryPlan& plan,
        ...
        ErrorCode DispatchMemoryQuery(const QueryPlan& plan,
                                      ...
                                      QueryExecutionResult* outResult)
        {
            ...
            if (plan.state == QueryPlanState::ScanFallback)
            {
                const ErrorCode fallbackRc = ExecuteControlledFallbackScan(
                    context.database, plan, context, &result);
            ...
            const ErrorCode collectRc = CollectCandidateRecords(
                context.database, plan, &matchedRecords, &result);
            if (Failed(collectRc))
            {
                result.unsupportedSource = QueryUnsupportedSource::Executor;
                result.mode = QueryExecutionMode::Unsupported;
