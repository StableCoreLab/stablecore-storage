...
sc::TablePtr CreateBeamTable(sc::DbPtr& db)
{
    ...
    length.defaultValue = sc::Value::FromDouble(0.0);
    length.indexed = true;
    EXPECT_EQ(schema->AddColumn(length), sc::SC_OK);

    sc::ColumnDef width = length;
    width.name = L"Width";
    EXPECT_EQ(schema->AddColumn(width), sc::SC_OK);

    sc::ColumnDef height = length;
...
TEST(StorageTableView, AggregateColumnTracksRelatedRecords)
{
    ...
    beamCount.aggregateKind = sc::AggregateKind::Count;
