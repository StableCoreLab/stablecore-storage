...
TEST(StorageM2Sqlite, CreateBackupCopyPreservesOrTrimsJournalHistory)
{
    ...
    EXPECT_EQ(CreateFileDb(preservedBackupPath.c_str(), preservedDb), sc::SC_OK);
    ...
    EXPECT_EQ(db->CreateBackupCopy(trimmedBackupPath.c_str(), trimmedOptions, &overwriteCheckResult), sc::SC_E_FILE_EXISTS);
}
