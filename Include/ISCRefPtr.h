...
namespace StableCore::Storage
{
    ...
    public:
        ...
        explicit SCRefPtr(T* ptr, bool addRef = true) noexcept : ptr_(ptr)
        {
        ...
        SCRefPtr(const SCRefPtr& other) noexcept : SCRefPtr(other.ptr_)
        {
        }

        template <class U>
            requires std::is_convertible_v<U*, T*>
        SCRefPtr(const SCRefPtr<U>& other) noexcept : SCRefPtr(other.Get())
        {
        }

        SCRefPtr(SCRefPtr&& other) noexcept : ptr_(other.ptr_)
        {
            other.ptr_ = nullptr;
        }

        template <class U>
        SCRefPtr(SCRefPtr<U>&& other) noexcept : ptr_(other.Detach())
        {
        }
