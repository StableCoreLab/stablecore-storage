...
namespace StableCore::Storage
{
    ...
    // their own executor implementations.
    ...
    // Query index inspection/rebuild is for query execution observability and
    // maintenance.
    ...
    // Reference index provider is intentionally source/target oriented; full
    ...
    // dispatcher.
    ErrorCode ExecuteQueryPlan(const QueryPlan& plan,
                               const QueryExecutionContext& context,
                               QueryExecutionResult* outResult);

    // Factory used by tests and bridge-facing code to obtain the default
