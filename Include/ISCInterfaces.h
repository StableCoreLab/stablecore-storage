...
namespace StableCore::Storage
{
    ...
    class SCQueryBridge;
    // Query planner and bridge remain compatibility seams only; backend
    // execution stays out of this header.
    struct QueryTarget;
    ...
    {
    public:
        // Stable logical identity of the record. The id is preserved across
        // undo/redo restore.
        virtual RecordId GetId() const noexcept = 0;
        // Deleted records remain addressable by existing handles but reject
        // writes and typed reads.
        virtual bool IsDeleted() const noexcept = 0;
        virtual VersionId GetLastModifiedVersion() const noexcept = 0;

        // Returns SC_OK when a SCValue exists, SC_E_VALUE_IS_NULL when the
    ...
    {
    public:
        // Returns both alive and deleted records when addressed by id. Deleted
        // state must be checked on the record.
    ...
    public:
        ...
        virtual ErrorCode GetLastConstraintViolationInfo(SCConstraintViolationInfo* outInfo) const
        {
            if (outInfo == nullptr)
            {
                return SC_E_POINTER;
