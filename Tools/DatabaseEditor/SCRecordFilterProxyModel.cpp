...
namespace StableCore::Storage::Editor
{
    ...
    bool SCRecordFilterProxyModel::filterAcceptsRow(
        int sourceRow, const QModelIndex& sourceParent) const
    {
        ...
        for (int column = 0; column < model->columnCount(sourceParent);
             ++column)
        {
    ...
    bool SCRecordFilterProxyModel::lessThan(const QModelIndex& left,
                                            const QModelIndex& right) const
    {
        ...
        return QString::localeAwareCompare(leftValue.toString(),
                                           rightValue.toString()) < 0;
    }
