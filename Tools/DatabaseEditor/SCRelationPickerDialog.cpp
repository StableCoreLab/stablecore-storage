...
namespace StableCore::Storage::Editor
{
          ...
          candidates_(candidates)
    {
        ...
        connect(filterEdit_, &QLineEdit::textChanged, this,
                &SCRelationPickerDialog::ApplyFilter);
        connect(tableWidget_, &QTableWidget::cellDoubleClicked, this,
                [this](int, int) { AcceptCurrentSelection(); });
        connect(buttonBox, &QDialogButtonBox::accepted, this,
                &SCRelationPickerDialog::AcceptCurrentSelection);
        connect(buttonBox, &QDialogButtonBox::rejected, this,
                &SCRelationPickerDialog::reject);
    ...
    void SCRelationPickerDialog::ApplyFilter(const QString& text)
    {
        for (int row = 0; row < tableWidget_->rowCount(); ++row)
        {
            ...
            if (!visible)
            {
                for (int column = 0; column < tableWidget_->columnCount();
                     ++column)
                {
                    const QTableWidgetItem* item =
                        tableWidget_->item(row, column);
                    if (item != nullptr &&
    ...
    void SCRelationPickerDialog::PopulateRows()
    {
        ...
        {
            const SCDatabaseSession::RelationCandidate& candidate =
                candidates_[row];
            QStringList preview;
            for (const auto& pair : candidate.previewFields)
            {
                ...
                preview.push_back(pair.first + QStringLiteral("=") +
                                  pair.second);
            }

            auto* idItem =
                new QTableWidgetItem(QString::number(candidate.recordId));
            idItem->setData(Qt::UserRole,
                            QVariant::fromValue<qlonglong>(candidate.recordId));
            tableWidget_->setItem(row, 0, idItem);
            tableWidget_->setItem(row, 1,
                                  new QTableWidgetItem(candidate.label));
            tableWidget_->setItem(
                row, 2,
                new QTableWidgetItem(preview.join(QStringLiteral(" | "))));
        }
